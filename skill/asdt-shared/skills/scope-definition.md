# Scope Definition — Shared Skill

## Purpose

This shared skill provides guidelines for defining explicit, unambiguous project scope. It is consumed by both the **Architect** and **Developer** specialists when they need to establish or validate scope boundaries for a feature or change.

Apply these guidelines when producing or reviewing `scope`, `nfrs`, and `open_questions` sections of an artifact.

---

## Explicit In/Out Lists

Scope creep begins with ambiguity. Every spec and plan must contain both:

- **`scope.in`**: A list of features, interactions, states, and behaviors that are explicitly within this change. Each item must be concrete enough that a developer or architect can decide whether a given task belongs here.
...
## Guidance for Architect Specialist

When you use this skill as the Architect, apply scope definition to the `system-design` artifact:
- `scope.in` covers architectural boundaries, service contracts, and API surface area.
- `scope.out` explicitly excludes implementation details owned by the Developer specialist.
- NFRs at the architectural level focus on scalability, reliability, and security contracts.
- Open questions surface design decisions that must be resolved before the Developer can proceed.

---

## Guidance for Developer Specialist

When you use this skill as the Developer, apply scope definition to the `implementation-plan` artifact:
- `scope.in` lists files to create/modify and implementation steps included in this plan.
- `scope.out` lists adjacent refactors or features that are explicitly deferred.
- NFRs at the implementation level focus on performance budgets, error handling contracts, and test coverage requirements.
- Open questions surface implementation decisions that block a step but have multiple valid approaches.
