...
Running `asdt init` creates `.asdt/config.yaml` with sensible defaults. Edit it to match your project.
...
Generated by `asdt init` (or `/asdt-init`). Describes the technical context of the project so every specialist can tailor its output without re-detecting the stack.
