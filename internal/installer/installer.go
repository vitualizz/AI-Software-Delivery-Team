...
type InstallResult struct {
	...
	Written     []string
	// WrittenCommands holds paths to extra discoverability artifacts
	// (e.g. OpenCode command-palette wrappers) generated by this
	// assistant's CommandAdapter, if any. It is additive and distinct
	// from Written (which holds copied skill-tree files) — an assistant
	// absent from CommandAdapters always has WrittenCommands == nil.
...
// generateCommands runs the assistant's CommandAdapter, if any, AFTER
// the skill-tree copy has already succeeded. It is strictly additive
// post-processing: an assistant absent from CommandAdapters (e.g.
// Claude Code) is left completely untouched — result stays
// byte-identical to the pre-adapter behavior (AC#2). When an adapter
// IS found, its written paths are appended to result.WrittenCommands
// and any error is surfaced via result.Err — without discarding paths
// that were written before the error (partial success, AC#5).
...
// commandRootFor derives the command-palette root directory for the
// given assistant. It is the one seam where a CommandAdapter's output
// location is resolved per-assistant — Generate itself stays
// path-agnostic and testable with any temp directory. Returns "" for
// an assistant with no known command root (defensive default; today
// every registered adapter has one).
