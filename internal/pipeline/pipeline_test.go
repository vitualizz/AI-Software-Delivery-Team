...
// TestAdvanceStep_OldAdvanceStillCompiles verifies that the existing v1 Advance
// method is still callable — backward compatibility.
func TestAdvanceStep_OldAdvanceStillCompiles(t *testing.T) {
	ctx := context.Background()
	m := newMachine(t)
	change := "backward-compat"

	// v1 Advance must still work.
	_, err := m.Advance(ctx, change, pipeline.PhaseRequirements)
	if err != nil {
